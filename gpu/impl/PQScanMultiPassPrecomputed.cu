...
loadPrecomputedTerm(LookupT* smem,
  ...
  if (numCodes % kWordSize == 0) {
    ...
    LookupVecT* term2StartV = (LookupVecT*) term2Start;
    LookupVecT* term3StartV = (LookupVecT*) term3Start;
    LookupVecT* normV = (LookupVecT*) norm;
...
#pragma unroll
      for (int j = 0; j < kUnroll; ++j) {
        LookupVecT q =
          LoadStore<LookupVecT>::load(&term3StartV[i + j * blockDim.x]);
         LookupVecT n =
          LoadStore<LookupVecT>::load(&normV[i + j * blockDim.x]);

        vals[j] = Math<LookupVecT>::add(vals[j], q);
        vals[j] = Math<LookupVecT>::add(vals[j], n);
      }
...
#pragma unroll
    ...
    for (int i = remainder + threadIdx.x; i < numCodes; i += blockDim.x) {
      smem[i] = Math<LookupT>::add(term2Start[i], term3Start[i]);
      smem[i] = Math<LookupT>::add(smem[i], norm[i]);
    }
  } else {
    // Potential unaligned load
    constexpr int kUnroll = 4;
...
#pragma unroll
      for (int j = 0; j < kUnroll; ++j) {
        vals[j] = Math<LookupT>::add(vals[j], term3Start[i + j * blockDim.x]);
        vals[j] = Math<LookupT>::add(vals[j], norm[i + j * blockDim.x]);
      }
...
template <typename LookupT, typename LookupVecT>
inline __device__ void
loadPrecomputedTermGraph(LookupT* smem,
                    LookupT* term2Start,
                    LookupT* term3Start,
                    int numCodes) {
  constexpr int kWordSize = sizeof(LookupVecT) / sizeof(LookupT);
...
#pragma unroll
      for (int j = 0; j < kUnroll; ++j) {
        ...
        vals[j] = Math<LookupVecT>::sub(vals[j], q);
      }
...
#pragma unroll
    ...
    for (int i = remainder + threadIdx.x; i < numCodes; i += blockDim.x) {
      smem[i] = Math<LookupT>::sub(term2Start[i], term3Start[i]);
    }
  } else {
    // Potential unaligned load
    constexpr int kUnroll = 4;
...
#pragma unroll
      for (int j = 0; j < kUnroll; ++j) {
        vals[j] = Math<LookupT>::sub(vals[j], term3Start[i + j * blockDim.x]);
      }
...
template <int NumSubQuantizers, typename LookupT, typename LookupVecT>
__global__ void
pqScanPrecomputedMultiPassGraph(Tensor<float, 2, true> queries,
                           Tensor<float, 2, true> precompTerm1,
  ...
  auto codesPerSubQuantizer = precompTerm2.getSize(2);
  auto precompTermSize = precompTerm2.getSize(1) * codesPerSubQuantizer;
   LookupT* smemIter = (LookupT*) smemTerm23;
  LookupT* term4 = smemIter;
  for(int iter=0;iter<nedge;iter++){
      ...
      //LookupT* term2 = precompTerm2[listId].data();
      //LookupT* term21 = precompTerm2[edgeId].data();
      ...
      float la =lambdaInfo[lambda[0]];
       float dist = term1+la*(term6)+(la*la-la)*term5;
    ...
    #pragma unroll
        for (int word = 0; word < kNumCode32; ++word) {
          ...
          if (kBytesPerCode32 == 1) {
            auto code = code32[0];
            dist += la*ConvertTo<float>::to(term4[code]);
    ...
    #pragma unroll
            for (int byte = 0; byte < kBytesPerCode32; ++byte) {
              ...
              dist += la*ConvertTo<float>::to(term4[offset + code]);
            }
          }
        }
...
pqScanPrecomputedMultiPassGraph(Tensor<float, 2, true> queries,
  ...
  auto precompTermSize = precompTerm2.getSize(1) * codesPerSubQuantizer;
   LookupT* smemIter = (LookupT*) smemTerm23;
   smemIter+=precompTermSize;
  LookupT* term4 = smemIter;
    // This is where we start writing out data
      // We ensure that before the array (at offset -1), there is a 0 value

      auto listId = coarseIndices2nd[queryId][probeId];

      int outBase = *(prefixSumOffsets[queryId][probeId].data() - 1);
      float* distanceOut = distance[outBase].data();
      int listId1st = listId/nedge;
      int iter = listId%nedge;
      auto edgeId = edgeInfo[listId1st][iter];
      // Safety guard in case NaNs in input cause no list ID to be generated
      if (listId == -1||listLengths[listId]==0) {
        return;
      }

      unsigned char* codeList = (unsigned char*) listCodes[listId];

      uint8_t* lambdaList = (uint8_t*) listLambdas[listId];

    int limit = min(listLengths[listId], 1024);
      //int limit = listLengths[listId];
      ...
      // Load precomputed terms 1, 2, 3
      float term1 = precompTerm1[queryId][probeId];
      float term5 = edgeDistInfo[listId1st][iter];
      float term6 = graphDistances[queryId][probeId];
      float* norm = subQuantizerNorms.data();
       LookupT* term2 =precompTerm2[edgeId].data();
       LookupT* term21=precompTerm2[listId1st].data();
      // if(queryId==0&&listId1st==3237&&iter==0){
       //    printf("term1:%.4f,term5:%.4f,term6:%.4f,outBase:%d,limit:%d",term1,term5,term6,outBase,limit);
      // }
      loadPrecomputedTerm<LookupT, LookupVecT>(term23,
                                              precompTerm2[listId1st].data(),
                                              precompTerm3[queryId].data(),
                                             // subQuantizerNorms.data(),
                                               precompTermSize);
     loadPrecomputedTermGraph<LookupT, LookupVecT>(term4,
                                              precompTerm2[edgeId].data(),
                                              precompTerm2[listId1st].data(),
    ...
    #pragma unroll
        for (int word = 0; word < kNumCode32; ++word) {
          ...
          if (kBytesPerCode32 == 1) {
            auto code = code32[0];
            dist += ConvertTo<float>::to(term23[code]);
            tmp +=  ConvertTo<float>::to(term4[code]);
    ...
    #pragma unroll
            for (int byte = 0; byte < kBytesPerCode32; ++byte) {
              ...
              tmp += ConvertTo<float>::to(term4[offset + code]);
            }
          }
        }
        ...
        // We do not maintain indices here, in order to reduce global
        // memory traffic. Those are recovered in the final selection step.
        distanceOut[codeIndex] = dist+la*tmp;
...
void
runMultiPassTileGraph(Tensor<float, 2, true>& queries,
                 Tensor<float, 2, true>& precompTerm1,
                   Tensor<float, 1, true>& subQuantizerNorms,
  ...
  runCalcListOffsetsGraph(coarseIndices2nd,listLengths, prefixSumOffsets,
                     thrustMem, stream);
     //checkPrefixSumOffsets(prefixSumOffsets.data(),prefixSumOffsets.numElements(), stream);
  ...
  // index) values for all intermediate results
  {
    ...
    auto grid = dim3(coarseIndices2nd.getSize(1),
                     coarseIndices2nd.getSize(0));
...
#endif
     smem *=2*numSubQuantizers * numSubQuantizerCodes;
...
#define RUN_PQ_OPT(NUM_SUB_Q, LOOKUP_T, LOOKUP_VEC_T)                   \
    do {                                                                \
      ...
      auto precompTerm3T = precompTerm3.toTensor<LOOKUP_T>();           \
                                                                 \
      pqScanPrecomputedMultiPassGraph<NUM_SUB_Q, LOOKUP_T, LOOKUP_VEC_T>     \
        <<<grid, block, smem, stream>>>(                                \
          ...
          edgeInfo,                                              \
          edgeDistInfo,                                           \
          lambdaInfo, constInfo,                                                \
          listCodes.data().get(),                                       \
          listLengths.data().get(),                                     \
          listLambdas.data().get(), listConsts.data().get(),                                    \
          prefixSumOffsets,                                             \
          allDistances);                                                \
    } while (0)
...
#undef RUN_PQ_OPT
 ...
 // outputVec("allDistances",allDistances.data(),10,stream);
   int nedge = edgeInfo.getSize(1);
  // k-select the output in chunks, to increase parallelism
  runPass1SelectLists(prefixSumOffsets,
                      allDistances,
                      coarseIndices2nd.getSize(1),
                      k,
                      false, // L2 distance chooses smallest
                      heapDistances,
                      heapIndices,
                      stream);
  //outputVec("prefixSumOffsets",heapDistances.data(),k,stream);
  //outputVecInt("heapIndices",heapDistances.data(),k,stream);
  ...
  runPass2SelectListsGraph(flatHeapDistances,
                      flatHeapIndices,
                      listIndices,
                      indicesOptions,
                      prefixSumOffsets,
...
runMultiPassTileGraph(Tensor<float, 2, true>& queries,
  ...
  // Calculate offset lengths, so we know where to write out
  // intermediate results
  runCalcListOffsetsGraph(topQueryToCentroid, edgeInfo.getSize(1),listLengths, prefixSumOffsets,
                     thrustMem, stream);
  // checkPrefixSumOffsets(prefixSumOffsets.data(),prefixSumOffsets.numElements(), stream);
      //outputVec("allDistancesBefore",allDistances.data(),10,stream);
  // Convert all codes to a distance, and write out (distance,
  // index) values for all intermediate results
  {
    auto kThreadsPerBlock = 256;
...
#undef RUN_PQ_OPT
  ...
  runPass1SelectLists(prefixSumOffsets,
                      allDistances,
                      topQueryToCentroid.getSize(1)*nedge,
                      k,
                      false, // L2 distance chooses smallest
                      heapDistances,
                      heapIndices,
                      stream);
  //outputVec("prefixSumOffsets",heapDistances.data(),k,stream);
 //outputVec("heapDistances11",heapDistances[0][0].data(),k,stream);
  // k-select final output
  auto flatHeapDistances = heapDistances.downcastInner<2>();
  auto flatHeapIndices = heapIndices.downcastInner<2>();
...
void runPQScanMultiPassPrecomputedGraph(Tensor<float, 2, true>& queries,
  ...
  size_t sizePerQuery =
    2 * // # streams
    ((nprobe  *nedge*sizeof(int) + sizeof(int)) + // prefixSumOffsets
     nprobe *nedge*maxListLength * sizeof(float) + // allDistances
     sizeForFirstSelectPass);
  ...
  // FIXME: we should adjust queryTileSize to deal with this, since
  // indexing is in int32
  FAISS_ASSERT(queryTileSize * nprobe*nedge * maxListLength <=
         std::numeric_limits<int>::max());
 ...
 // Make sure there is space prior to the start which will be 0, and
  // will handle the boundary condition without branches
  DeviceTensor<int, 1, true> prefixSumOffsetSpace1(
    mem, {queryTileSize * nprobe*nedge + 1}, stream);
  DeviceTensor<int, 1, true> prefixSumOffsetSpace2(
    mem, {queryTileSize * nprobe*nedge + 1}, stream);

  DeviceTensor<int, 2, true> prefixSumOffsets1(
    prefixSumOffsetSpace1[1].data(),
    {queryTileSize, nprobe*nedge});
  DeviceTensor<int, 2, true> prefixSumOffsets2(
    prefixSumOffsetSpace2[1].data(),
    {queryTileSize, nprobe*nedge});
  DeviceTensor<int, 2, true>* prefixSumOffsets[2] =
  ...
  DeviceTensor<float, 1, true> allDistances1(
    mem, {queryTileSize * nprobe *nedge* maxListLength}, stream);
  DeviceTensor<float, 1, true> allDistances2(
    mem, {queryTileSize * nprobe *nedge* maxListLength}, stream);
  DeviceTensor<float, 1, true>* allDistances[2] =
    {&allDistances1, &allDistances2};
  ...
  for (int query = 0; query <queries.getSize(0); query += queryTileSize) {
    ...
    auto term1View =
      precompTerm1.narrowOutermost(query, numQueriesInTile);
    auto graphDistancesView =
      graphDistances.narrowOutermost(query, numQueriesInTile);
    auto term3View =
      precompTerm3.narrowOutermost(query, numQueriesInTile);
    ...
    runMultiPassTileGraph(queryView,
                     term1View,
                     subQuantizerNorms,
                     precompTerm2,
                     term3View,
...
void runPQScanMultiPassPrecomputedGraph(Tensor<float, 2, true>& queries,
                                  ...
                                  Tensor<float, 1, true>& subQuantizerNorms,
                                   ...
                                   Tensor<long, 2, true>& outIndices,
                                   GpuResources* res) {
  constexpr int kMinQueryTileSize = 4;
  constexpr int kMaxQueryTileSize = 128;
  constexpr int kThrustMemSize = 16384;
  ...
  for (int query = 0; query <queries.getSize(0); query += queryTileSize) {
    ...
    auto coarseIndicesView =
      topQueryToCentroid.narrowOutermost(query, numQueriesInTile);
       ...
       auto coarseIndices2ndView =
      coarseIndices2nd.narrowOutermost(query, numQueriesInTile);
    auto queryView =
      queries.narrowOutermost(query, numQueriesInTile);
    auto term1View =
      precompTerm1.narrowOutermost(query, numQueriesInTile);
    ...
    // outputVecInt("deviceListLengths_",listLengths.data().get(),100,stream);
    runMultiPassTileGraph(queryView,
                     term1View,
                     subQuantizerNorms,
                     precompTerm2,
