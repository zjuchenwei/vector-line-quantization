...
runUpdateListPointers(Tensor<int, 1, true> listIds,
                      ...
                      int* listLengths,
                      void** listCodes,
                      void** listIndices, void** listLambdas, void** listConsts) {
  ...
  listLambdas[listId] = newLambdaPointers[index];
  listConsts[listId] = newConstPointers[index];
}
...
runUpdateListPointers(Tensor<int, 1, true>& listIds,
  ...
  runUpdateListPointers<<<grid, block, 0, stream>>>(
    listIds, newListLength, newCodePointers, newIndexPointers,newLambdaPointers,newConstPointers,
    listLengths.data().get(),
    listCodes.data().get(),
    listIndices.data().get(),listLambdas.data().get(),listConsts.data().get());
...
ivfpqInvertedListAppend(Tensor<int, 1, true> listIds,
 ...
 // auto encoding = encodings[encodingToAdd];
  ...
  uint8_t lambda = lambdas[encodingToAdd][0];
  ((uint8_t*) listLambdas[listId])[offset]=lambda;

   //uint8_t constq = consts[encodingToAdd][0];
  //((uint8_t*) listConsts[listId])[offset]=constq;
...
#define RUN_APPEND(IND)                                         \
  do {                                                          \
    ivfpqInvertedListAppend<IND><<<grid, block, 0, stream>>>(   \
      listIds, listOffset, encodings, indices,lambdas, consts,                 \
      listCodes.data().get(),                                   \
      listIndices.data().get(),listLambdas.data().get(),listConsts.data().get());                                \
  } while (0)
