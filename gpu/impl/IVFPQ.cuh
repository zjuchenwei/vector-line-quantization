...
class IVFPQ : public IVFBase {
 public:
  ...
  void addCodeVectorsFromCpu(int listId,
                             ...
                             const long* indices,
                             size_t numVecs);
 void addCodeVectorsFromCpu(int listId,size_t numVecs);
void addLambdasFromCpu(int listId,
                             const void* lambdas,
                             size_t numVecs);
...
void
queryGraph(Tensor<float, 2, true>& queries,
...
void queryGraph1(Tensor<float, 2, true>& queries,
             Tensor<int, 2, true>&  devEdgeInfo,
             Tensor<float, 2, true>&  devEdgeDistInfo,
             Tensor<float, 1, true>&  devLambdaInfo,
              Tensor<float, 1, true>&  devConstInfo,
   ...
   void testGraph(Tensor<float, 2, true>& queries,
             Tensor<int, 2, true>&  devEdgeInfo,
             Tensor<float, 2, true>&  devEdgeDistInfo,
             Tensor<float, 1, true>&  devLambdaInfo,
             int nprobe,
  ...
  void testPQPrecomputedCodesGraph_(
  ...
  std::vector<unsigned char> getListCodesCPU(int listId) const;
 ...
 public:
  ...
  DeviceTensor<float, 3, true> precomputedCode_;
    DeviceTensor<float, 1, true> subQuantizerNorms_;
#ifdef FAISS_USE_FLOAT16
  /// Precomputed term 2 in half form
  DeviceTensor<half, 3, true> precomputedCodeHalf_;
   DeviceTensor<half, 1, true> subQuantizerNormsHalf_;
#endif
};
