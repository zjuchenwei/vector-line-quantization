...
IVFBase::IVFBase(GpuResources* resources,
    ...
    space_(space),
    dim_(quantizer->getDim()),
    numLists_(quantizer->getSize()*numedge),
    maxListLength_(0) {
  reset();
}
...
IVFBase::reset() {
  ...
  deviceListLambdas_.clear();
   deviceListConsts_.clear();
  deviceListDataPointers_.clear();
  deviceListIndexPointers_.clear();
  deviceListLambdaPointers_.clear();
  deviceListConstPointers_.clear();
  deviceListLengths_.clear();
  listOffsetToUserIndex_.clear();
  listOffsetToUserCode_.clear();
  for (size_t i = 0; i < numLists_; ++i) {
    ...
    deviceListIndices_.emplace_back(
      std::unique_ptr<DeviceVector<unsigned char>>(
        new DeviceVector<unsigned char>(space_)));
       deviceListLambdas_.emplace_back(
      std::unique_ptr<DeviceVector<unsigned char>>(
        new DeviceVector<unsigned char>(space_)));
        deviceListConsts_.emplace_back(
      std::unique_ptr<DeviceVector<unsigned char>>(
        new DeviceVector<unsigned char>(space_)));
    listOffsetToUserIndex_.emplace_back(std::vector<long>());
    listOffsetToUserCode_.emplace_back(std::vector<unsigned char>());
  }

  deviceListDataPointers_.resize(numLists_, nullptr);
  deviceListIndexPointers_.resize(numLists_, nullptr);
  deviceListLambdaPointers_.resize(numLists_, nullptr);
  deviceListConstPointers_.resize(numLists_, nullptr);
...
IVFBase::updateDeviceListInfo_(const std::vector<int>& listIds,
  ...
  HostTensor<void*, 1, true>
    hostNewLambdaPointers({(int) listIds.size()});
   HostTensor<void*, 1, true>
    hostNewConstPointers({(int) listIds.size()});
  ...
  for (int i = 0; i < listIds.size(); ++i) {
    ...
    auto& lambdas = deviceListLambdas_[listId];
    auto& consts = deviceListConsts_[listId];

    auto& data1 =listOffsetToUserCode_[listId];
    hostListsToUpdate[i] = listId;
    hostNewListLength[i] = data1.size()/bytesPerVector_;
    hostNewDataPointers[i] = data->data();
    hostNewIndexPointers[i] = indices->data();
    hostNewLambdaPointers[i] = lambdas->data();
    hostNewConstPointers[i] = consts->data();
  }
  ...
  DeviceTensor<void*, 1, true> newLambdaPointers(
    mem, hostNewLambdaPointers, stream);
  DeviceTensor<void*, 1, true> newConstPointers(
    mem, hostNewConstPointers, stream);
