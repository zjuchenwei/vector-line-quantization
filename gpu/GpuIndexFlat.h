...
/** return the indexes of the k vectors closest to the query x.
     ...
     */
    void assignFlat (idx_t n, const float * x, int * labels, idx_t k = 1);
     ...
     */
    void compute_residual(faiss::Index::idx_t n,const float * x,
            float * residual,
            int* edgeInfo,
            float* lambda,
...
void compute_residual(faiss::Index::idx_t n,const float * x,
 ...
 /// `x`, `distances` and `labels` can be resident on the CPU or any
  ...
  /// copies; searchImpl_ is not called
  /// FIXME: move paged impl into GpuIndex
  /// assign1 (n, x, assign.data(),assign1.data(),lamda.data());
  void assign1(
      faiss::Index::idx_t n,
      int d,
      const float* x,
      ...
      float* lamdaf,
      int* edgeinfo,
      float* edgedistinfo,int nlist,int numedge, int k=1) const;
...
void assign1Base(
      ...
      Tensor<float,2,true> edgedistinfoV,int k=1) const;
  ...
  inline FlatIndex* getGpuData() { return data_; }
   ...
   void searchNonPagedWithGraph_(int n,
                              const float* x,
                              int k,
                              int nlist,
                              int edge,
 ...
 void buildGraphNonPaged_(int n,
                       int k,
                       float* outDistancesData,
                       int* outIndicesData) const;
