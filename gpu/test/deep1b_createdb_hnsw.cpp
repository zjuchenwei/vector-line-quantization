...
uint8_t* readUint8(const char* _fn, size_t _dim, size_t _num, size_t _offset) {
  ...
  //INDICES_64_BIT = 3,
    ...
    { // training
        ...
        if (!file_exists(codebook_file)) {
         ...
         const std::string prename1   = "centroids_deep1b_" + std::to_string(d) + "_" + std::to_string(nlist)
                                   + "_" + std::to_string(nsub);
    ...
    { printf ("[%.3f s] Loading database\n", elapsed() - t0);
        ...
        int chuncksize = 1000000;
         const std::string prename2   = "precomputed_idxs_deep1b_" + std::to_string(d) + "_" + std::to_string(nlist)
        ...
        for(int iter = 0;iter<100000000;iter+=chuncksize){
             xb = readFloat("/home/data/deep1B/base.umem", d2, chuncksize, iter);
