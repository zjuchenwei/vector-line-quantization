...
int main ()
{
  ...
  //INDICES_64_BIT = 3,
...

    {
        ...
        index.readDbFromFile(prename,300000*1000);
        printf ("[%.3f s] done\n", elapsed() - t0);
    }
    uint nq;
    float *xq;
...

    {
        ...
        //int *gt_int = ivecs_read("/home/data/deep1B/truth.ivecs", &kgt, &nq2);
        	...
        	header("deep300M.imem", nq2, kgt);
        	std::cout << "nq2" << nq2 << "kgt" << kgt<< std::endl;;
	       int *gt_int = new int[kgt * nq2];
//	read<int>(path_truth, GT, gt_dim, gt_num);
	     read<int>("deep300M.imem", gt_int, kgt * nq2, 0);
