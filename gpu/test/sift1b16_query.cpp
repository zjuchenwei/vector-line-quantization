...
void quickSort(float *s,faiss::Index::idx_t *idx, int l, int r)
{
	if (l< r)
	{
		int i = l, j = r, x = s[l],y=idx[l];
		while (i < j)
		{
		...
		s[i] = x;
        idx[i]=y;
		quickSort(s, idx,l, i - 1); // 递归调用
		quickSort(s, idx,i + 1, r);
	}
}
  ...
  //INDICES_64_BIT = 3,
    ...
    faiss::gpu::GpuIndexIVFPQ index (
      ...
      std::string prename   = _name+ "_" + std::to_string(d) + "_" + std::to_string(nlist)
       ...
       // }
       // else{
       //      prename="/home/data/sift1b/"+prename;
    ...
    { // training
        ...
        index.readCodebookFromFile(prename);
            ...
            std::cout << "codebook not exists, reading from " << codebook_file << std::endl;
            return 0;
          }
    }
...

    {
        ...
        printf ("[%.3f s] done\n", elapsed() - t0);
            ...
            std::cout << "db not exists, reading from " << db_file << std::endl;
            return 0;
          }
    }
    uint nq;
    ...
    { // searching the database
        ...
        index.setNumProbes(nlist/1024);
         index.w1_=nlist*nedge*0.375/1024;
