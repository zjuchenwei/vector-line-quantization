...
void IndexIVFPQ::search1(idx_t n, const float *x, idx_t k,
                              float *distances, idx_t *labels) const
{
    long * idx = new long [n * nprobe];
    ScopeDeleter<long> del (idx);
    ...
    indexIVFPQ_stats.assign_cycles += TOC;
      ...
      for (int q = 0; q < n; ++q) {
        ...
        for (int r = 0; r <nprobe; ++r) {
          ...
          long listId = idx[q*nprobe+r];
          auto& listIndices = ids[listId];
           size_t list_size = listIndices.size();
...
void IndexIVFPQ::histogram(uint _nBins) const{
	std::vector<uint> binHist;
	binHist.resize(8);
	uint total = 0;
    std::ofstream outFile;
	outFile.open("imi-sift1b.csv", std::ios::out); // 打开模式可省略
	uint maxVal = 0;
	uint maxIdx = 0;
	for (int i = 0; i < _nBins; i++) {
        auto& listIndices = ids[i];
           size_t list_size = listIndices.size();
          outFile<< list_size<<",";
    ...
    std::cout << "<300 \t" << binHist[2] << std::endl;
     ...
     for(int a =0 ; a<8;a++){
                 std::cout << binHist[a]/static_cast<double>(_nBins) << ",";
        }
    std::cout << std::endl;
