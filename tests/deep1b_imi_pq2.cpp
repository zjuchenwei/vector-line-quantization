...
int main ()
{
	...
	"/home/data/deep1B/learn.umem";
    float *xt = readFloat(path_learn, d, 2000000, 0);
    //---------------------------------------------------------------
    ...
    //
    size_t nhash = 2;
    size_t nbits_subq = 12;
    size_t ncentroids = 1 << (nhash * nbits_subq);  // total # of centroids
    int bytes_per_code = 8;
    std::string _name= "deep1b_" +std::to_string(nbits_subq)+"_" +std::to_string(bytes_per_code);
    ...
    // faiss::Index * idx = faiss::read_index("/tmp/trained_index.faissindex");
        ...
        header("/home/data/deep1B/base.umem", nb, d2);
        ...
        for (size_t begin = 0; begin <nb; begin += add_bs) {
           ...
           float* xb = readFloat("/home/data/deep1B/base.umem", d2, chunksize, begin);
...
{ // searching the database
      ...
      for (size_t iter = 0; iter < nq; iter+= tileSize) {
        ...
        pIndex->nprobe = 2048*4;
        pIndex->search1(curNum, xq+iter*d, k, dis, nns);
 ...
 //evaluate result by hand.
         for(int a =0 ; a<18;a++){
             ...
             for(int a =0 ; a<17;a++){
                 std::cout << 1.0f * correct[a] / nq << ",";
            }
            std::cout <<  1.0f * correct[17] / nq <<  std::endl;
